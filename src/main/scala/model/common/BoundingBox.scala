...
object BoundingBox {
  ...
  /**
   * Rectangle bounding box is used by [model.reaction.SimultaionObjectImpl.LeavesOfPlantsImpl]
   * and [predatorImpl].
   * @param point  of Point2D class represent the center of the Rectangle.
   * @param width  of the Rectangle Bounding box.
   * @param height of the  Rectangle Bounding box
   */

  case class Rectangle(point: Point2D, width: Int, height: Int) extends BoundingBox

  /**
   * Triangle bounding box represent creature food [model.reaction.SimultaionObjectImpl.FlowerPlant]
   * and nectar.
   * @param point  of Point2D class represent the center of the Circle.
   * @param angle  of the Triangle Bounding box.
   * @param height of the  Triangle Bounding box
   */

  case class Triangle(point: Point2D, height: Int, angle: Double = DEF_EQUILATERAL_ANGLE) extends BoundingBox

  /** Apply constructors method of every bounding boxes */
...
object Intersection {
 ...
 //takes triangle as a circle, calculating the radius of circle circumscribed in the triangle
  ...
  // verify that there is no gap between any of the 4 sides of the rectangles, takes as 2 rectangle collision
  /**
   *
   * @param circle is body1.
   * @param rectangle is body2.
   * @return true if no gap, false otherwise.
   */
